...
- (void)viewDidLoad
{
     ...
     */
    self.filterResults =
        [NSMutableArray arrayWithCapacity:[self.dataController countOfLocationList:self.dataController.stationList]];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self.dataController setSortedStationList:[self.dataController sortLocationList:self.dataController.stationList
                                                                           byMethod:LocationDataSortByName]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_searchController.isActive) {
        ...
        // If we are not performing a search, this whole table is just comprised of cells representing the
        // sortedStationList
        locationAtIndex = (Station *)[self.dataController objectInLocationList:self.dataController.sortedStationList
        ...
        case DestinationTableSectionStations: {
            if (_searchController.isActive) {
                ...
                self.selectedLocation =
                    (MyLocation *)[[[self dataController] sortedStationList] objectAtIndex:[indexPath row]];
                [self showNavigateActions:self.selectedLocation.name];
            }
            break;
        }
...
- (void)updateFilteredContentForLocationName:(NSString *)locationName type:(NSString *)typeName
{
     ...
     */
    self.searchLocation = [[MyLocation alloc] initWithName:locationName
                                                coordinate:CLLocationCoordinate2DMake(0, 0)
                                          distanceFromUser:CLLocationDistanceMax];

    /*
     Search the main list for locations whose type matches the scope (if selected) and whose name matches searchText;
                 ...
                 completionHandler:^(NSArray *placemarks, NSError *error) {
                     ...
                     for (CLPlacemark *placemark in placemarks) {
                         DLog(@"Placemark name: %@ subthoroughfare: %@ thoroughfare: %@ sublocality: %@ locality: %@ "
                              ...
                              placemark.name, placemark.subThoroughfare, placemark.thoroughfare, placemark.subLocality,
                              placemark.locality, placemark.subAdministrativeArea, placemark.administrativeArea);
                         Address *address = [[Address alloc]
                             initWithPlacemark:placemark
                              distanceFromUser:MKMetersBetweenMapPoints(
                                                   MKMapPointForCoordinate(placemark.location.coordinate),
                                                   MKMapPointForCoordinate(self.userCoordinate))];
